...
class Paymentgateway extends Omnipay {
	...
	public function __construct($set_gateway='PayPal_Pro',$test_mode=true){
		...
		$PaymentData = $CI->DatabaseOperationsHandler->_get_row_data('tbl_payment_settings',array('paymentgateway_id'=>$set_gateway['gateway']));
		$this->gateway = Omnipay::create($set_gateway['gateway']);
		
		switch ($set_gateway['gateway']) {
			case 'PayPal_Pro':
