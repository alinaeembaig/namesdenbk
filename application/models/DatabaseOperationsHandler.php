...
class DatabaseOperationsHandler extends CI_Model
{
    ...
    public function getUserData($user_id){
        ...
        if(!empty($query->result_array())){
            ...
            $this->db->where('username',urldecode($user_id));
            $query = $this->db->get('tbl_users');
            return $query->result_array();
        }
    }
    ...
    public function _get_my_contracts($open=true,$mode='direct',$limit=""){
        $ignore = array(0,1,2,3,5,6,8,9,10,11);
        if($open){
            $ignore = array(4, 7);
        } 
        ...
        if($mode === 'direct'){
            $this->db->group_start();
            $this->db->where('contract_method',$mode);
            $this->db->or_where('contract_method','inhouse');
            $this->db->group_end();
        }
        else
        {
        ...
        if(!empty($limit)){
            $query = $this->db->get('tbl_opens',$limit);   
        }
        else
        {
    ...
    /* Return Row Data Array*/
    public function _get_row_data_pagination($table_name,$data,$status_limit=false,$or_condition='',$limit='', $start=''){
        if(!empty($data)){
            $this->db->group_start();
            $this->db->where($data);
            if(!empty($or_condition)){
    ...
    /*Insert to Table*/
    public function _save_to_table($table,$data){
    ...
    /*Payment Plugin Data */
    public function _update_payment_plugins($data){
        $this->db->where('id',$data['id']);
        $q = $this->db->get('tbl_payment_settings');

        if ( $q->num_rows() > 0 ) 
        {
          $this->db->where('id',$data['id']);
          return $this->db->update('tbl_payment_settings',$data);
        } 
        else 
        {
          return $this->db->insert('tbl_payment_settings',$data);
        }
    }
    ...
    public function _userwise_all_listings($userid, $type='',$limit='', $start='' ,$count=false , $sort = 'website_BusinessName'){
        $Arr = array();
        ...
        switch ($sort) {
            case 'name':
                $this->db->order_by('website_BusinessName', "asc");
                break;
            case 'recent':
                $this->db->order_by('date', "desc");
                break;
            case 'lowest':
                $this->db->order_by('website_buynowprice', "asc");
                break;
            case 'highest':
                $this->db->order_by('website_buynowprice', "desc");
                break;
            default:
                $this->db->order_by('website_BusinessName', "asc");
                break;
    ...
    public function _get_all_offers($listing_id,$status='0',$type,$sort="tbl_offers.offer_amount",$order='DESC',$reviews="",$owner='on'){
        ...
        if(!empty($dataArr)){
            ...
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago']                             = timespan(human_to_unix($key['offer_date']), $now, 1).' ago';
    ...
    public function _get_all_bids($listing_id,$status='1',$type,$sort="tbl_bids.bid_amount",$order='DESC',$reviews="",$owner='on'){
        ...
        if(!empty($dataArr)){
            $i=0;
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago']                             = timespan(human_to_unix($key['bid_date']), $now, 1).' ago';
    ...
    public function _userwise_offers($userid,$group_by=""){
        $this->db->select('*,COUNT(tbl_offers.id) as NOF,MAX(tbl_offers.offer_amount) as maxAmount,(tbl_listings.id) as listing_id,(tbl_listings.status) as listing_status , (tbl_offers.date) as offerdate ');
        ...
        if(!empty($dataArr)){
            $i=0;
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago'] = timespan(human_to_unix($key['offerdate']), $now, 1).' ago';
                $i++;
            }
        }
        return $dataArr;
    ...
    public function _userwise_bids($userid,$group_by=""){
        $this->db->select('*,COUNT(tbl_bids.id) as NOF,MAX(tbl_bids.bid_amount) as maxAmount,(tbl_listings.id) as listing_id,(tbl_listings.status) as listing_status , (tbl_bids.date) as biddate');
        ...
        if(!empty($dataArr)){
            $i=0;
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago'] = timespan(human_to_unix($key['biddate']), $now, 1).' ago';
                $i++;
            }
        }
        return $dataArr;
    ...
    public function _get_userwise_bids($listing_id,$user_id,$group=''){
        ...
        $this->db->select('*,(tbl_bids.id) as bid_id ,(tbl_bids.date) as biddate, (tbl_listings.date) as listing_date,(tbl_listings.status) as listing_status ,tbl_listings.website_BusinessName,tbl_listings.website_tagline,(tbl_bids.listing_type) as type');
        $this->db->join('tbl_users', 'tbl_users.user_id = tbl_bids.bidder_id'); 
        ...
        if(!empty($dataArr)){
            ...
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago']     = timespan(human_to_unix($key['biddate']), $now, 1).' ago';
                $dataArr[$i]['expire']  = timespan($now , strtotime($key['listing_date']. ' + '.$settingsData[0]['auction_period'].'days') , 3) !== '1 Second' ? timespan($now , strtotime($key['listing_date']. ' + '.$settingsData[0]['auction_period'].'days') , 3).' to end' : 'Auction Ended';
                $i++;
            }
        }
    ...
    public function _get_userwise_offers($listing_id,$user_id,$group=''){
        $this->db->select('*,(tbl_offers.id) as offer_id , (tbl_offers.date) as offerdate');
        ...
        if(!empty($dataArr)){
            ...
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago']     = timespan(human_to_unix($key['offerdate']), $now, 1).' ago';
                $dataArr[$i]['expire']  = timespan($now , human_to_unix($key['expire_date']) , 3) !== '1 Second' ? timespan($now , human_to_unix($key['expire_date']) , 3).' to expire' : 'Listing Expired';
                $i++;
            }
        }
        return $dataArr;
    ...
    public function _get_comments($listing_id,$type='listing'){
        ...
        if(!empty($dataArr)){
            $i=0;
            foreach ($dataArr as $key) {
                $dataArr[$i]['ago'] =  timespan(human_to_unix($key['created_at']), $now, 1).' ago';
                $i++;
            }
        }
        return $dataArr;
    ...
    public function _get_auction_ending_soon($type='',$void=array()){
    ...
    public function _get_sponsored_listing($listingType='sponsored',$limit=false,$type="all"){
        $data['options']     =   $this->_get_row_data('tbl_platforms',array('type'=>'option','status'=>1));
    ...
    public function _count_listings_categories_wise($listing_type='website'){
        ...
        $key = array_search("domain",$platforms);
        if (!$key) {
            unset($platforms[$key]);
        }
        ...
        $this->db->join('tbl_purchases', 'tbl_purchases.plan_id = tbl_listings.id AND tbl_purchases.expire_date >='.$this->db->escape($today).' AND tbl_purchases.purchase_date <='.$this->db->escape($today).' AND tbl_purchases.listing_type IN ('.implode(",",$this->db->escape(array_column($platforms, 'platform'))).') AND tbl_listings.status = 1', 'right');
        $this->db->join('tbl_categories', 'tbl_categories.id = tbl_listings.website_industry AND tbl_listings.status= 1 AND `tbl_listings`.`listing_type` IN ('.implode(",",$this->db->escape(array_column($platforms, 'platform'))).') AND tbl_categories.c_level = 0','right'); 
    ...
    public function _get_selected_listing_types($type,$sold=0,$limit=50,$listing='',$void=array('app','account')){
        ...
        if(!empty($listingsArr)){
            ...
            foreach ($listingsArr as $listing) {
                ...
                else if($listing['listing_type'] === 'app')
                {
                    $listingsArr[$i]['category']        =   $listing['listing_type'];
                    $listingsArr[$i]['categoryIcon']    =   'app.svg';
                }
                else if($listing['listing_type'] === 'account')
                {
                    $listingsArr[$i]['category']        =   $listing['listing_type'];
                    $listingsArr[$i]['categoryIcon']    =   'account.svg';
                }
                else
                {
                    $listingsArr[$i]['category']        =   $listing['listing_type'];
                ...
                else
                {
                 ...
                 $listingsArr[$i]['ago']                =   timespan(human_to_unix($listing['date']), $now, 1).' ago';
    ...
    public function _get_contract($id){
        ...
        $this->db->select('*,DATE(tbl_opens.delivery_time) as dateonly,TIME(tbl_opens.delivery_time) as timeonly,tbl_opens.date as date');
    ...
    public function _search_table($data,$limit,$start,$sort,$arr='',$count=false){
        ...
        $this->db->select('*,(tbl_listings.id) AS id , (tbl_listings.date) AS date');
        if(!empty($arr)){
        ...
        $this->db->join('tbl_purchases', 'tbl_purchases.plan_id = tbl_listings.id AND tbl_purchases.expire_date >='.$this->db->escape($today).' AND tbl_purchases.purchase_date <='.$this->db->escape($today).' AND tbl_purchases.listing_type IN ('.implode(",",$this->db->escape(array_column($platforms, 'platform'))).')', 'right');
        ...
        if(!empty($dataArr)){
            ...
            foreach ($dataArr as $key) {
                ...
                $dataArr[$i]['username']    = $this->getUserData($key['user_id'])[0]['username'];
                $dataArr[$i]['ago']         = timespan(human_to_unix($key['date']), $now, 1).' ago';
                $dataArr[$i]['sell_type']   = $dataArr[$i]['listing_option'];  
                $dataArr[$i]['sell_web']    = $dataArr[$i]['listing_type']; 
            ...
            $userListings[$i]['listingType']                = $listing['listing_type'];
            {
                ...
                if(isset($this->_get_highest_bid_details('1',$listing['id'],$listing['listing_type'])[0]['bid_amount'])){
                    ...
                    if($userListings[$i]['highestbidrow'] > 0){
                        $userListings[$i]['highestbid']        = $this->database->_get_current_price($listing['id'],$listing['listing_type']);
                    }
                    else
                    {
                        $userListings[$i]['highestbid']        = $this->database->_get_current_price($listing['id'],$listing['listing_type']);
    ...
    /*Get All Telegrams Data*/
    public function _get_telegram_data($data=array(),$limit,$count=false,$start){
        $today = date('Y-m-d H:i:s');
        $this->db->select('*,(tbl_listings.id) AS id');   
        $this->db->join('tbl_purchases', 'tbl_listings.id = tbl_purchases.plan_id');
        $this->db->join('tbl_platform_list', 'tbl_listings.extension = tbl_platform_list.platfrom_domain');
        $this->db->group_start(); 
        $this->db->where('tbl_purchases.listing_type = tbl_listings.listing_type');
        $this->db->where('tbl_purchases.expire_date>=', $today);
        $this->db->where('tbl_purchases.purchase_date <=', $today);
        ...
        $this->db->group_start(); 
        $this->db->where('tbl_listings.listing_type','account');
        $this->db->group_end();
        ...
        $this->db->group_by('tbl_listings.id');//mod v2.6
        $this->db->order_by('tbl_listings.date','desc');
        $query = $this->db->get('tbl_listings');
        return $query->result_array();
    }
    ...
    public function _get_offers_data($condition,$limit,$count=false,$start){
        ...
        if(!empty($userListings)){
            ...
            foreach ($userListings as $listing) {
                ...
                $userListings[$i]['ago']                    = timespan(human_to_unix($listing['date']), $now, 1).' ago';
