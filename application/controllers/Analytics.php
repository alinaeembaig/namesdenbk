...
class analytics extends CI_Controller {
    ...
    public function index($domain="",$reverse="",$backtype=""){
        ...
        if($this->DatabaseOperationsHandler->CheckAlreadyExists('tbl_domains',$datachk,$this->session->userdata('user_id')) > 0){
            $data['settings']   =   $this->database->_get_row_data('tbl_settings',array('id'=>1));
            if($data['settings'][0]['active_domain_verification'] !== '0'){
              exit('Unauthorized access');
            }
        }
    ...
    public function oauth2callback()
    {
      ...
      else 
      {
          ...
          if($datas['settings'][0]['active_domain_verification'] !== '0'){
            $status = $this->DatabaseOperationsHandler->_update_to_table('tbl_domains',array('google_token'=>json_encode($client->getAccessToken())),array('id'=>$this->session->userdata('domain'),'status'=>1));
          }
          else
          {
            $status = $this->DatabaseOperationsHandler->_update_to_table('tbl_domains',array('google_token'=>json_encode($client->getAccessToken())),array('id'=>$this->session->userdata('domain'),'status'=>0));
          }
    ...
    /*Google Revoke Access Token*/
	  public function unlink($domain,$reverse=""){
        ...
        $datas['settings']   =   $this->database->_get_row_data('tbl_settings',array('id'=>1));
        if($datas['settings'][0]['active_domain_verification'] !== '0'){
          $status = $this->DatabaseOperationsHandler->_update_to_table('tbl_domains',$data,array('id'=>$domain,'status'=>1));
        }
        else
        {
          $status = $this->DatabaseOperationsHandler->_update_to_table('tbl_domains',$data,array('id'=>$domain,'status'=>0));
        }
    ...
    public function getMonthlyWiseViews($domain){
      ...
      if (isset($access_token) && $access_token) {
        ...
        {
          $domainData   = $this->DatabaseOperationsHandler->_get_row_data('tbl_domains',array('id'=>$domain,'status'=>0));
        }
  ...
  /*Retrive Google Adsense Data according to domain listings*/
  public function getAdenseMonthlydata($id,$type='all'){
    $output['token']       = $this->security->get_csrf_hash();
    header('Content-Type: application/json');
    $analytics = $this->AnalyticsOperationsHandler->getAdenseMonthlydata($id,$type);
    if($analytics){
        $output['response']         = $analytics;
        exit(json_encode($output));
    }
