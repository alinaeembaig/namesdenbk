...
public function send_msg(){
    ...
    {
        $output['response']      = false;
        $output['message']       = "Captcha does not match.";
        exit(json_encode($output));
    }       
}
...
public function readAndVerifyDNS(){
    ...
    if(!empty($this->input->post('dataArr'))){
        ...
        try
        {
            ...
            /*$dns  = new Dns();
            $txtRecords = $dns->getRecords($dnsdata['domain'], DNS_TXT);
            foreach ($txtRecords as $txt) {
                if (!strcmp($txt->txt(),$dnsdata['token'])) {
                    $response = true;
                    return true;
                }
            }*/

            $dns  = new Dns($dnsdata['domain']);
            $txtRecords = $dns->getRecords('TXT');

            if (str_contains($txtRecords, $dnsdata['token'])) {
                $response = true;
            }
            else
            {
...
public function get_selected_row($table,$column,$value){
    ...
    public function RegisterUser(){
        ...
        if ($captcha_insert === $contain_sess_captcha) {
            ...
            $output['response']  = false;
            $output['message']   = "Sorry registration fail";
            exit(json_encode($output));
    ...
    {
        /*$config = require FCPATH . '/phpagebuilder/config/config.php';
        $builder = new PHPageBuilder\PHPageBuilder($config);
        $builder->handleRequest();*/

        //$installationFolder = FCPATH . '/phpagebuilder';
