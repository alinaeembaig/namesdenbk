...
public function manage_videos(){
    $data = self::$data;
    $data['platforms']  =   $this->database->_get_row_data('tbl_platforms',array('type'=>'listing'));
    $data['urls']  =   $this->database->_get_row_data('tbl_videos',array());
...
public function save_ads(){
    ...
    /*Save FAQ */
    public function save__faq_data(){
        $output['token'] = $this->security->get_csrf_hash();
        header('Content-Type: application/json');

        $data = array(
            'category'=>$this->input->post('listing_type'),
            'heading' =>$this->input->post('heading'),
            'body' =>$this->input->post('body'),
        );

        $this->form_validation->set_data($data);
        $this->form_validation->set_rules('heading', 'FAQ Heading', 'required|trim|xss_clean');
        $this->form_validation->set_rules('body', 'FAQ Body', 'required|trim|xss_clean');

        if ($this->form_validation->run()){
            if(!empty($this->input->post('faq_id'))){
                $output['response'] = $this->database->_update_to_table('tbl_faq',$data,array('id'=>$this->input->post('faq_id')));
                exit(json_encode($output));
            }
            else
            {
                $output['response'] = $this->database->_insert_to_table('tbl_faq',$data);
                exit(json_encode($output));
            }   
        }
    ...
    /*Save Videos */
    public function save__videos_data(){
        $output['token'] = $this->security->get_csrf_hash();
        header('Content-Type: application/json');
        ...
        $this->form_validation->set_data($data);
        $this->form_validation->set_rules('url', 'video URL', 'required|trim|xss_clean');

        if ($this->form_validation->run()){
            if(!empty($this->input->post('video_id'))){
                $output['response'] = $this->database->_update_to_table('tbl_videos',$data,array('id'=>$this->input->post('video_id')));
                exit(json_encode($output));
            }
            else
            {
                $output['response'] = $this->database->_insert_to_table('tbl_videos',$data);
                exit(json_encode($output));
            }   
        }
