...
class user extends CI_Controller {
	...
	function __construct() {
		...
		self::$data['paymentsOptions']                  =   $this->database->_get_row_data('tbl_payment_settings',array());
	...
	public function manage_bidders($type,$id){
		...
		if(!empty($id)){
			...
			if(!empty($data['listing_data'][0]['domain_id']) || in_array($type,array('app','account'))) {
	...
	public function manage_bids($type,$id){
		...
		if(!empty($id)){
			...
			if(!empty($data['listing_data'][0]['domain_id']) || in_array($type,array('app','account'))) {
				...
				$data['seller_info']					=	$this->database->getUserData($data['bids'][0]['owner_id']);
				$data = html_escape($this->security->xss_clean($data));
	...
	public function manage_listings(){
		...
		$sort = ($this->uri->segment(3)) ? $this->uri->segment(3) : 'name';

		$config = array();
        $config["base_url"] 					= base_url() . "user/manage_listings/".$sort;
        $config["total_rows"] 					= $this->database->_userwise_all_listings($this->session->userdata('user_id'),'auction','','',true,$sort);
        $config["per_page"] 					= USER_LISTINGS;
		...
		$data['websitelistings'] = 	$this->_userwise__listings($this->session->userdata('user_id'),'auction','','','',$config["per_page"],$page,$sort);
	...
	public function manage_offers(){
		...
		$config = array();
        $config["base_url"] 					= base_url() . "user/manage_offers/".$sort;
        $config["total_rows"] 					= $this->database->_userwise_all_listings($this->session->userdata('user_id'),'classified','','',true,$sort);
        $config["per_page"] 					= USER_OFFERS;
        $config["uri_segment"] 					= 4;
		...
		$data['websitelistings'] = 	$this->_userwise__listings($this->session->userdata('user_id'),'classified','','','',$config["per_page"],$page,$sort);
	...
	/*Upload Images */
	public function upload__multiple_image($nameBox,$path=IMAGES_UPLOAD){
        ...
        {           
        	...
        	$image_data = $this->upload->data();
			$dataInfo[$i] = $image_data['file_name'];
        }
	...
	public function create__listings($type,$id=""){
		if(!empty($type) && $type ==='website'){
			...
			$data = self::$data;
			$data['listingType'] 						=  	$type;
			$data['platforms']                        	=   $this->database->_get_row_data('tbl_platforms',array('type'=>'listing','status'=>1,'platform'=>$type));
			$data['marketplaces']                       =   $this->database->_get_row_data('tbl_platform_list',array('status'=>1,'listing_type'=>$type));
			$data['listingOptions']						=	$this->database->_get_row_data('tbl_listing_header',array('listing_type'=>$type));
			$data['sponsorOptions']						=	$this->database->_get_row_data('tbl_listing_header',array('listing_type'=>'sponsored'));
			if(is_numeric($id)){
				$data['listing_data']					=	$this->database->_get_row_data('tbl_listings',array('id'=>$id,'listing_type'=>$type,'status'=>0,'user_id'=>$this->session->userdata('user_id')),'',false);
			...
			$data = html_escape($this->security->xss_clean($data));
			$this->load->view('user/create-any-listings',$data);
			return;
		}
		$this->pageNotFound();
	}
	...
	public function edit_listings($type,$id){
		if(!empty($type) && !empty($id) && $type =='website'){
			...
			if(!empty($data['listing_data'][0]['website_BusinessName'])) {
				...
				$this->load->view('user/edit-any-listings',$data);
				return;
			}
		}
		$this->pageNotFound();
	...
	/*Incomplete Listings*/
	public function incomplete_listings(){
		$data = self::$data;
		$data = html_escape($this->security->xss_clean($data));
		$this->load->view('user/incomplete-listings',$data);
		return;
	}
	...
	public function open_contract(){
		...
		if(empty($this->input->post('offer_id'))){
			...
			if(isset($bid[0]['id'])){
				...
				if($this->input->post('contract-method') === 'escrow'){
					$tid = $this->escrow->_create_transaction_contract($this->input->post('o_bid_id_cont'),'bid','broker');
					return;
				}
				...
				$data = array(
					...
					'contract_method'=>$this->input->post('contract-method')
				);
		...
		else
		{
			...
			if(isset($offer[0]['id'])){
				...
				if($this->input->post('contract-method') === 'escrow'){
					$tid = $this->escrow->_create_transaction_contract($this->input->post('offer_id'),'offer','broker');
					return;
				}
	...
	public function contract($id){
		...
		if(!empty($id)){
			...
			if(isset($data['contract'][0]['bid_id'])){
				...
				if($data['contract'][0]['contract_method'] === 'escrow'){
					$data['t_id']				= 	$this->database->_get_single_data('tbl_contracts',array('contract_id'=>$data['contract'][0]['id']),'invoice_id');
					$data['escrow_transaction']	= 	$this->escrow->_read_transaction($data['t_id']);

					if($data['escrow_transaction']['items'][0]['schedule'][0]['status']['secured']) { 
						$this->common->change_contract_status($data['contract'][0]['id'],11);
						$this->common->change_delivery_date($data['contract'][0]['id'],11);
						$this->database->_update_to_DB('tbl_listings',array('sold_status' => 1),$data['listing_data'][0]['id']);
					}

					if($data['escrow_transaction']['items'][0]['status']['accepted'] && !empty($data['contract'][0]['id'])) {
						$this->common->change_contract_status($data['contract'][0]['id'],4);
						$this->database->_update_to_table('tbl_invoices',array('status'=>8,'date'=>date('Y-m-d H:i:s')),array('invoice_id'=>$data['t_id']));
					}

					if($this->session->userdata('user_id') === $data['contract'][0]['customer_id']){
						$data['role']			= 'buyer';
						$data['payment_methods']= $this->escrow->_get_payment_methods($data['t_id']);
					}
					else if($this->session->userdata('user_id') === $data['contract'][0]['owner_id']){
						$data['role']	= 'seller';
					}
					else
					{
			...
			if($data['contract'][0]['contract_method'] === 'escrow'){
				$this->load->view('user/open-escrow',$data);
				return;
			}
			else if($data['contract'][0]['contract_method'] === 'direct'){
				$this->load->view('user/open-contract',$data);
				return;
			}
			else if($data['contract'][0]['contract_method'] === 'inhouse'){
				$this->load->view('user/open-contract',$data);
				return;
			}
		}
	...
	public function _userwise__listings($userid,$type='',$limit=false,$sold=false,$listing_id='',  $limits='' , $start='' , $sort=''){
		if(!$limit){

			$userListings = $this->database->_userwise_all_listings($userid,$type,$limits,$start, false ,$sort);
		} 
		else
		{
			if(empty($listing_id)){
	...
	public function add_listing(){
		...
		if($this->input->post('listing_type') === 'domain'){
			...
			$sliderImages = $this->sliderUpload();
			...
			$data = array(
				...
				'screenshot' => json_encode($sliderImages),
				'website_cover' => "",
				'status' => 0,
				'sold_status' => 0,
				...
				'date' => date('Y-m-d H:i:s'),
				'token' => '',
				'godaddy_link' => $this->input->post('godaddy_link'),
				'dan_link' => $this->input->post('dan_link'),
			);
	...
	public function delete__listing($id){
		if($this->database->_delete_from_table('tbl_listings', array('id'=>$id,'user_id'=>$this->session->userdata('user_id')))){
			redirect(base_url().'user/incomplete_listings');
			return;
		}
		else
		{
	...
	public function sliderUpload()
	{
    	...
    	$dataInfo = array();
    	$files = $_FILES;
    	$cpt = count($_FILES['imageSliders']['name']);
    	for($i=0; $i<$cpt; $i++)
    	{           
        	$_FILES['imageSlider']['name']= $files['imageSliders']['name'][$i];
        	$_FILES['imageSlider']['type']= $files['imageSliders']['type'][$i];
        	$_FILES['imageSlider']['tmp_name']= $files['imageSliders']['tmp_name'][$i];
        	$_FILES['imageSlider']['error']= $files['imageSliders']['error'][$i];
        	$_FILES['imageSlider']['size']= $files['imageSliders']['size'][$i];    

        	$this->upload->initialize($this->set_upload_options());
        	$this->upload->do_upload('imageSlider');
