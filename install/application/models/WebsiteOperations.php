	...
	public function finalProcess()
	{
		...
		if(!$error) {
			...
			if($this->migrate()){
				...
				$sampleConfigFile = file_get_contents("assets/samples/config.sample");
				$sampleConfigFile = str_replace("{{base_url}}",preg_replace("(^https?://)","",$basePathValue),$sampleConfigFile);
				file_put_contents("../application/config/config.php",$sampleConfigFile);
