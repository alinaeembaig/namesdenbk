...
/*--------------------------------------------------*/
/*  Listing Adsense Info
/*--------------------------------------------------*/
"use strict";	
function loadAdsenseData(id,type='line',adtype='all',ticks=true){
	Chart.defaults.global.defaultFontFamily = "Nunito";
	Chart.defaults.global.defaultFontColor = '#888';
	Chart.defaults.global.defaultFontSize = '14';
	...
	console.log($('#listingidwebsite').val());

	$.ajax({   
		method :'GET',
		url:baseUrl+'analytics/getAdenseMonthlydata/'+$('#listingidwebsite').val()+'/'+adtype,
		dataType: 'json',
		success:function(data){ 
			console.log(data);
			$('.txt_csrfname').val(data.token);	
			...
			scales: {
				yAxes: [{
					...
					ticks: {
                    	display: ticks //this will remove only the label
                	},
				}],
				xAxes: [{
					scaleLabel: { display: false },  
...
function getLabels(response){
	...
	{
		arrLabels[i]	= response[i][1].substring(0, 3);
	}
	return arrLabels;
}
