...
/*--------------------------------------------------*/
    $(document).on('click','#BtnAnyNext',function(){
    	...
    	if (form.valid()) {
    		...
    		bootstrap_alert.success(lang_dom_val_wait,'#ContinueVal');
...
/*--------------------------------------------------*/
    $(document).on('click','#BtnNextFinal',function(){
    	...
    	if(sponsoreditemArr){
    		itemArrFull.push(sponsoreditemArr);
    		build_checkout_summary(currency_code);
    	}
    				...
    				success: function (data) {
    					if(data !==''){
    						...
    						if(data.response !== false){  
    							...
    							+'<strong> ('+lang_dom_val_line_0+') </strong><br>'
    							+'1). '+lang_dom_val_line_1+' : &nbsp;<a href="'+baseUrl+'/assets/verification/'+data.response['token']+'.zip'+'" class="btn btn-success">DOWNLOAD FILE</a><br>'
    							+'2). '+lang_dom_val_line_2+'<br>'
    							+'3). <b>'+lang_dom_val_line_3+'</b>'
    							...
    							+'4). '+lang_dom_val_line_4+' '
    							+'<button id="btnVerifyDomain" name="btnVerifyDomain" type="button" class="btn btn-info centerButtons margin-bottom-10">'+lang_dom_val_veify+'</button>'
    							+'</div></br>'
    							+'<strong> ('+lang_dom_val_line_5+') </strong><br>'
    							+'1). <strong>'+lang_dom_val_line_6+' :</strong> &nbsp;'+data.response['token']+'.'+data.response['domain']+'<br>'
    							+'2). <strong>'+lang_dom_val_line_7+' : </strong>'+data.response['token']+'<br>'
    							+'3). '+lang_dom_val_line_8+'<br>'
    							+'<strong>'+lang_dom_val_line_9+'</strong><br>'
    							+'<button id="btnDnsVerifyDomain" name="btnDnsVerifyDomain" type="button" class="btn btn-info centerButtons margin-bottom-10">'+lang_dom_val_veify+'</button>'
    							+'</div></br>'
    							+'';
    							...
    							else
    							{
    								...
    								$("#domainVerificationDiv").hide();
    								bootstrap_alert.success(lang_dom_val_success,'#DomainValMsg');
    								...
    								$.ajax({
    									...
    									success: function (data) {
    										...
    										if(data.response === true){
    											bootstrap_alert.error(lang_dom_val_already_exist,'#DomainValMsg');
    							...
    							$("#loadingImageVerify").hide();
    							bootstrap_alert.error(updateError,'#DomainValMsg');
    							return;
    						}
    					}
    				}
...
$(document).on('click','.button-verify-listurl',function(){
	...
	CheckPlatformURL($("#listURL").val(),function(response) {
		if(response === false){
			bootstrap_alert.error(lang_dom_val_not_support,'#AppUrlValMsg');
			$("#loadingImageVerify").hide();
			return;
		}
		else
...
$(document).on('submit','#payWrapper',function(e){
	...
	if(!$('input[name="cardType"]:checked').val()){
		popnotificaton(errorSelectPayment,'danger');
		return false;
	}
...
$(document).on('submit','#createDomainsForm',function(e){
	...
	if($('#adminsiteURL').val() === '') {
		$('#loader').hide();
		bootstrap_alert.error(errorEmptyFields,'#notification');
		return;
	}
	...
	if($('input[name=website_1_group_2]:checked', form).val() === 'auction') {
		if ($('#website_reserveprice').length > 0) {
			if($('#website_reserveprice').val() !=='' && $('#website_startingprice').val() !== ''){
				if(parseFloat($('#website_reserveprice').val()) < parseFloat($('#website_startingprice').val())){
					$('#loader').hide();
					bootstrap_alert.error(lang_dom_val_reserved_price,'#notification');
					return;
				}
			}
		}	
	...
	if (form.valid()) {
		...
		$.ajax({
			...
			success:function(data){
				...
				{
					bootstrap_alert.error(data.error,'#categoriesSettingsMsg');
					$('#loadingCategories').hide(); 
					return;
				}
			},
		...
		success: function (data) {
			...
			if(data !==''){
				...
				else
				{
					$("#loadingImageVerify").hide();
					$("#btnfoward").hide();
					bootstrap_alert.error(lang_dom_val_failed,'#DomainValMsg');
					return;
				}
...
$(document).on('click','#btnDnsVerifyDomain',function(){
	var csrfName = $('.txt_csrfname').attr('name');
	var csrfHash = $('.txt_csrfname').val();
	$("#loadingImageVerify").show();
	var values = $('#savedDataInfo').val(); 
	...
	$.ajax({
		url: baseUrl+'common/readAndVerifyDNS/',
		type:"POST",
		data: {dataArr : values,[csrfName]: csrfHash},
		dataType: "json",
		success: function (data) {
...
$('input[type=radio][name=listing_group_1]').change(function() {
	...
	$('#btnfoward').hide();
	itemArrFull = [];
	bootstrap_alert.success(lang_dom_val_wait,'#PayCalVal');
	$.ajax({
		method :'POST',
...
function build_checkout_summary(currency_code = '$'){
	var total = 0;
	var items = "";
	var totalOutput;

	for (var i = 0; i < itemArrFull.length; i++) {
		total += parseFloat(itemArrFull[i]['listing_price']);
		items += '<div class="item">'
		+'<span class="price">'+ currency_code + ' ' +itemArrFull[i]['listing_price']+'</span>'
		+'<p class="item-name">'+itemArrFull[i]['listing_name']+'</p>'
		+'<p class="item-description">'+itemArrFull[i]['listing_description']+'</p>'
		+'</div>';
	}
	
	totalOutput = '<div class="total">Total<span class="price">'+ currency_code +' '+ total +'</span></div>';

	$('#txt_payamount').val(total);
	$('#LoadingPayCalVal').hide();
	$('#btnfoward').show(); 
	$('.checkout-summary').html(items + totalOutput);
}
...
$(document).on('change', '#website_reserveprice', function(e){  
	...
	if(this.value !=='' && $('#website_startingprice').val() !== ''){
		if(parseFloat(this.value) < parseFloat($('#website_startingprice').val())){
			bootstrap_alert.error(lang_dom_val_reserved_price,'#reservredPriceWebsite');
			return;
		}
	}
});
...
$(document).on('change', '#domain_reserveprice', function(e){  
	...
	if(this.value !=='' && $('#domain_startingprice').val() !== ''){
		if(parseFloat(this.value) < parseFloat($('#domain_startingprice').val())){
			bootstrap_alert.error(lang_dom_val_reserved_price,'#reservredPriceHelp');
			return;
		}
	}
});
...
$('.popup-with-accept-offer').click(function(){
	...
	$('.escrow_email_buyer').val($(this).data('escrow'));
	$('.escrow_email_seller').val($(this).data('seller-escrow'));
	$('#offer-note').html(lang_model_accepting_offer + $(this).data('bidder')+"</b> "+ lang_as_per_our + " <a href='"+baseUrl+"page/terms-of-service'> "+ lang__model_terms_conditions + "</a> "+ lang_model__offer_price +" <b>"+$(this).data('bidcur')+addCommas($(this).data('bidamount')));
	$('#bid_amount').html($(this).data('bidcur')+addCommas($(this).data('bidamount')));
});
...
$('.popup-with-accept-bidders').click(function(){
	...
	$('#offer-from').html(lang_model_accept_bids_from +$(this).data('bidder'));
							...
							$.ajax({
								...
								success:function(data){
									...
									else
									{
										if(data.response === false){
											$('.captcha-refresh').click();
											bootstrap_alert.error(data.message,'#registrationStatus');
											return;
										}
...
/*--------------------------------------------------*/
						$(document).on('click','#btn_paypal',function(e){
							e.preventDefault();
							$('#loader').show();
							$('#pgateway_setup_form').submit();
						});

/*--------------------------------------------------
| Delete Payment Gateway
/*--------------------------------------------------*/
						$(document).on("click", ".payment_remove", function(e){
							e.preventDefault();
							var csrfName = $('.txt_csrfname').attr('name');
							var csrfHash = $('.txt_csrfname').val();
							var id = $(this).data("id");
							$.ajax({
								url:baseUrl+'admin/delete_from_table/'+id,
								method:"POST",
								data:{[csrfName]: csrfHash},
								dataType: 'json',
								success:function(data){
...
/*--------------------------------------------------*/
						$( '#plugin_upload_form' ).submit( function( e ) {
							e.preventDefault();
							var data = new FormData(this);
							$('#loadingImagePlugins').show();
							$.ajax( {
								url: baseUrl+'admin/upload_plugins',
								type: 'POST',
								data: data,
								processData: false,
								contentType: false,
								success:function(data){
									if(!data)
									{
										bootstrap_alert.success('Extension Installed Sucessfully','#pluginUploadmsg');
										$("#table_payments_div").load(location.href + " #table_payments_div");
										return;
									}
									else
									{
										bootstrap_alert.error(data,'#pluginUploadmsg');
										return;
									}
								},
								complete: function(){
									$('#loadingImagePlugins').hide();
								},
							...
							$.ajax({
								...
								success:function(data){
									...
									else
									{
										$('.captcha-refresh').click();
										$('#loader').hide(); 
										bootstrap_alert.error(data.message,'#notification');
									}
								},
								complete: function(){
									$('#loader').hide(); 
						...
						function checkoutpage(){
							$(function() {
								$(".creditly-card-form .submitpay").click(function(e) {
									...
									if(!$('input[name="cardType"]:checked').val()){
										bootstrap_alert.error(errorSelectPayment,'#paymentValidations');
										return false;
									}
									
									if ($('.creditly-wrapper .'+$('input[name="cardType"]:checked').val()).length > 0) {
										var paypal = Creditly.initialize(
											'.creditly-wrapper .'+$('input[name="cardType"]:checked').val()+' .expiration-month-and-year',
											'.creditly-wrapper .'+$('input[name="cardType"]:checked').val()+' .credit-card-number',
											'.creditly-wrapper .'+$('input[name="cardType"]:checked').val()+' .security-code',
											'.creditly-wrapper .'+$('input[name="cardType"]:checked').val()+' .card-type');
									}
									...
									if($('input[name="cardType"]:checked').val() !== 'Escrow'){
										...
										if ($('.creditly-wrapper .'+$('input[name="cardType"]:checked').val()).length > 0) {
											...
											if (output) {
												...
												$('.txt_number').val(output.number);
												$('.txt_security-code').val(output.security_code);
												$('#paymentForm').submit();
											}
											return false;
										}
									...
									else
									{
										if($('#escrow_buyer_email').val()){
											if($('#owner_escrow').val()){
												...
												{
													popnotificaton(lang_dom_escrow_same,"danger");
												}
											}
											else
											{
												popnotificaton(lang_dom_escrow_pay_op,"warning");
											}
										}
										else
										{
											popnotificaton(lang_dom_escrow_no_email,"warning");
										}
									}
							...
							$("body").on("creditly_client_validation_error", function(e, data) {
								popnotificaton(data["messages"].join(", "),'danger');
							});
						}
...
/*  Customizations From
/*--------------------------------------------------*/
						$('#CustomizationsForm').on('submit',function(e){
							e.preventDefault();
							$('#loader').show();
							$.ajax({
								url:baseUrl+'admin/save_custom',
								type: 'POST',
								data:$("#CustomizationsForm").serialize(), 
								dataType: 'json',  
								success:function(data){
									$('.txt_csrfname').val(data.token);  
									if(data.response === true){
...
/*--------------------------------------------------*/
						...
						$(document).on('submit','#FAQForm',function(e){
							...
							if (form.valid()) {
								...
								$.ajax({
									method :'POST',
									url: baseUrl+'admin/save__faq_data',
									data:formData,
									dataType: 'json',
									cache:false,
									contentType: false,
...
| Edit Faqs
/*--------------------------------------------------*/
						$(document).on("click", ".editFaqs", function(e){
							e.preventDefault();
							var csrfName = $('.txt_csrfname').attr('name');
							var csrfHash = $('.txt_csrfname').val();
							var id = $(this).data("id");
							$.ajax({
								url:baseUrl+'common/get_selected_row/tbl_faq/id/'+id,
								method:"POST",
								data:{[csrfName]: csrfHash},
								dataType: 'json',
								success:function(data){
									$('.txt_csrfname').val(data.token);
									if(data.response !== false){
										document.getElementById("heading").value = data.response[0].heading;
										document.getElementById("faq_id").value = data.response[0].id;
										document.getElementById("body").value = data.response[0].body;
										$('#listing_type').val(data.response[0].category).change();  
									}
								},
								complete: function(){
								}
...
| Delete FAQs
/*--------------------------------------------------*/
						$(document).on("click", ".deleteFaqs", function(e){
							e.preventDefault();
							var csrfName = $('.txt_csrfname').attr('name');
							var csrfHash = $('.txt_csrfname').val();
							var id = $(this).data("id");
							$.ajax({
								url:baseUrl+'common/delete_from_table/tbl_faq/id/'+id,
								method:"POST",
								data:{[csrfName]: csrfHash},
								dataType: 'json',
								success:function(data){
...
/*--------------------------------------------------*/
						...
						$(document).on('submit','#videoForm',function(e){
							...
							if (form.valid()) {
								...
								$.ajax({
									method :'POST',
									url: baseUrl+'admin/save__videos_data',
									data:formData,
									dataType: 'json',
									cache:false,
									contentType: false,
...
/*--------------------------------------------------*/
						$(document).on("click", ".editVideo", function(e){
							...
							var id = $(this).data("id");
							$.ajax({
								url:baseUrl+'common/get_selected_row/tbl_videos/id/'+id,
								method:"POST",
								data:{[csrfName]: csrfHash},
								dataType: 'json',
								success:function(data){
									$('.txt_csrfname').val(data.token);
									if(data.response !== false){
										document.getElementById("video_id").value = data.response[0].id;
										document.getElementById("url").value = data.response[0].url;
										$('#listing_type').val(data.response[0].category).change();  
									}
								},
								complete: function(){
...
/*--------------------------------------------------*/
						$(document).on("click", ".deleteVideo", function(e){
							...
							var id = $(this).data("id");
							$.ajax({
								url:baseUrl+'common/delete_from_table/tbl_videos/id/'+id,
								method:"POST",
								data:{[csrfName]: csrfHash},
								dataType: 'json',
								success:function(data){
